...
Recall the angular velocity tracking error $ e_\Omega $, from this follows
...
\end{aligned}
\end{equation}
Equation \ref{eq:app.eOmega} is used to derive the attitude velocity error $ \dot{e}_R $ as follows
\begin{equation}\label{key}
\begin{aligned}
...
\end{aligned}
\end{equation}
It can be shown that $ \parallel C(R_d^TR)\parallel_2\leq 1 $ for any $ R_d^TR\in SO(3) $.\\ 
From that follows $ \parallel\dot{e}_R\parallel\leq\parallel e_\Omega \parallel $.
