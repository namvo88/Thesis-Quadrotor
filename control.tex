...
In contrast to hybrid control systems \cite{Gillula2010}, complicated reachability set analysis is not required to guarantee safe switching between different flight modes, as the region of attraction for each flight mode covers the configuration space almost globally. Global nonlinear dynamics of various classes of closed loop attitude control systems have been studied in recent years \cite{Chaturvedi2011a}. 
...
\section{Error Functions}\label{sec:con.configerr}
The control of a trajectory tracking problem requires state feedback and a measure of the error defining the difference between the current states and the desired states.
Since the system dynamics evolve on nonlinear manifolds, which describe the configuration spaces of the \a{qr} attitude $ \in SO(3) $ and the load attitude $ \in \mathbb{S}^2 $, error functions can also be defined on these same manifolds \cite{Bullo2005}. 
...
\subsection*{Quadrotor Attitude Error}
...
%\end{equation}}
where the \textit{vee map} $ ^\vee:\mathfrak{so}(3)\rightarrow\mathbb{R}^3 $ is the inverse of the \textit{hat map} defined in Section \ref{sec:mod.geometric}. From this equation, the \a{qr} attitude tracking error $ e_R \in \mathbb{R}^3$ is chosen as 
...
In Figure \ref{fig:con.transport}, two curves $ R(t) $ and $R_d(t)$ evolve on manifold $ SO(3) $. Transport map $ \mathcal{T}(R,R_d):T_{R_d}SO(3)\mapsto T_RSO(3) $ allows comparison of the velocity curves $ \dot{R} $ and $ \dot{R}_d $.
\begin{figure}[h!]
	\centering
...
\end{figure}		
%The tracking error $ \Psi_R $ and the transport map $ \mathcal{T}(R,R_d) $ allow the definition of error between velocity curves.

The time derivative of the tracking error function is defined by the tracking error function $ \Psi_R $, the transport map $ \mathcal{T}(R,R_d)  $ and the velocity error curve $ \dot{e} $. Where $ \dot{e} $ is a vector field along $ R $, which defines
the velocity error between $ R $ and $ R_d $, and is defined as
\begin{equation}\label{eq:con.dote}
\dot{e}=\dot{R}-\dot{R}_d(R_d^TR)
\end{equation}
This equation can be rewritten to obtain the angular velocity tracking error, as follows
\begin{equation}\label{key}
\begin{aligned}
...
\subsection*{Load Attitude Error}
...
e_q=\hat{q}^2q_d
\end{equation}
Again, a \textit{transport map} is used for a comparison between the tangent vectors on tangent spaces $ T_q\mathbb{S}^2$ and $ T_{q_d}\mathbb{S}^2$. Using the tracking error $ \Psi_q $ and the transport map $ \mathcal{T}_{\mathcal{S}^2}(q,q_d) $ the load angular velocity error function is defined as
\begin{equation}\label{eq:con.edq}
e_{\dot{q}}=\dot{q}-(q_d\times\dot{q}_d)\times q
\end{equation}

\subsection*{Load Position Error}
...
b_{3c}=\frac{F}{||F||}
\end{equation}
Such that $ F $ in Equation \ref{eq:con.b3c} is defined by a normal component $ F_n $, a proportional-derivative component $ F_{pd} $ and feedforward control force $ F_{ff}$
\begin{equation}\label{key}
F=F_n-F_{pd}-F_{ff}
\end{equation}
...
If the initial conditions in Equations \ref{eq:dom1}, \ref{eq:dom2}, \ref{eq:dom3} and \ref{eq:dom4} are satisfied, it is shown that 
It is proven in \cite{Sreenath2013c} that 
