...
%In Section \ref{sec:con.configerr}, 
...
\textit{Geometric control theory} is the study on how geometry of the state space influences control problems. 
...
% of the existing inner structure. By 
% that must stabilize the inner structure.
%This allows control of a new state, to guarantee the stability of the inner structure. 
%The control law is designed by using states as virtual control inputs, 
%such that 
...
\begin{figure}[h!]
	...
	\caption{Nonlinear geometric control loop of the \a{qr}-load system\label{fig:con.loop}}
\end{figure}	

The backstepping approach is able to calculate the control inputs $ f $ and $ M $ that are required to stabilize the \a{qr}, while several controllers are able to track different states, see Figure \ref{fig:con.loop} \cite{Sreenath2013c}. 
...
\begin{outline}
	\1 \a{qr} Attitude controlled Mode 
	\2 Track a desired \a{qr} attitude $ R_d(t) $ or commanded signal $ R_c(t) $
...
\begin{figure}[h!]
	...
	\caption{\a{qr} Attitude controller\label{fig:con.qrattalone}}
\end{figure}		
