...
The control law is designed by using states as virtual control signals. Each loop computes a virtual command signal for an inner loop, denoted by $ q_c $ and $ R_c $, being the computed load attitude and \a{qr} attitude, respectively.
% CHECK nodig?
...
Recall that $ R $ is the rotation matrix for the \a{qr} attitude, and $ R_d $ is the desired rotation matrix. Then, the attitude error is denoted as $ R^T_dR $, and describes the relative rotation from the body frame to the desired frame. 
...
The tangent vectors $ \dot{R} $ and $ \dot{R}_d $ cannot be compared directly, since they do not lie in the same space. $ \dot{R} $ and $ \dot{R}_d $ are expressed in their own tangent spaces, denoted by $ T_RSO(3)$ and $ T_{R_d}SO(3)$, respectively.\\
%In order to define an error function, 
$ \dot{R}_d $ is transformed into a vector on $ T_RSO(3) $ to compare it with $ \dot{R} $. This is done by an mathematical object called a \textit{transport map}, which enables the comparison of tangent vectors living in different spaces. \\
...
The load attitude error function on $ \mathbb{S}^2 $ represents the distance from the direction $ q $ to the desired direction $ q_d $, and is given by 
...
\end{equation}
%CHECK
% and a heading direction $ b_{1_d}(t) $. 

From Equations \ref{eq:mod.R} and \ref{eq:mod.attitude}, the derivative of the velocity tracking error in Equation \ref{eq:con.eOmega} can be written as 
\begin{equation}\label{key}
\begin{aligned}
...
Now the control input $ M $ can be defined as a proportional term, a derivative term and a canceling term, as follows \cite{Lee2010}
...
From Equation \ref{eq:con.dotRd} follows the desired angular velocity $ \dot{\Omega}_d $
\begin{equation}\label{key}
\begin{aligned}
