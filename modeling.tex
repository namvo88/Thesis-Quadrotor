...
The configuration of the \a{qr} can be described by the location of the \a{qr}s \a{cm}, $x_Q\in \mathbb{R}^3 $, described in the Euclidean space w.r.t. \IF and by the \textit{attitude} which is the orientation of \BF w.r.t. \IF evolving on a nonlinear space, described by a rotation matrix $R\in SO(3) $. The dynamics of the \a{qr} can be expressed on $ SE(3) $, which is the group of \textit{rigid displacements} in $ \mathbb{R}^3 $. A rigid displacement describes both the rotation and the position of \BF relative to \IF. \\

\begin{figure}[h!]
	\centering
	\makebox[\textwidth][c]{\includegraphics[width=.5\paperwidth]{./StyleStuff/qrmodel2.png}}
...
\end{bmatrix}
\end{equation}
where \lsymb{$ l $}{Distance from the rotor to the QR CM} is the distance from the rotor to the \a{qr}'s \acf{cm} and $ M_\phi, M_\theta, M_\psi $ denote the moments around the $ x, y, z $-axis in \BF, respectively. 
...
x_Q=x_L-Lq
\end{equation}
where \lsymb{$ x_Q \in\mathbb{R}^3$}{Position of the \a{qr} CM} is the position of the \a{qr}'s \a{cm} expressed in \IF, \lsymb{$ x_L\in\mathbb{R}^3 $}{Position of the load} is the position of the load expressed in \IF, and \lsymb{$ L $}{Length of the cable} is the length of the cable.
\begin{figure}[h!]
	\centering
...
\end{align}
where Equations \ref{eq:mod.loadpos} and \ref{eq:mod.loadatt} are the load position and attitude dynamics, and Equation \ref{eq:mod.qratt} are the \a{qr} attitude dynamics.\\
